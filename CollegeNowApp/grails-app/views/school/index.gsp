		...
		<div id="list-school" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="USD" title="${message(code: 'school.USD.label', default: 'USD')}" />
					
						<g:sortableColumn property="name" title="${message(code: 'school.name.label', default: 'Name')}" />
					
						<g:sortableColumn property="schoolID" title="${message(code: 'school.schoolID.label', default: 'School ID')}" />
				...
				<g:each in="${schoolInstanceList}" status="i" var="schoolInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${schoolInstance.id}">${fieldValue(bean: schoolInstance, field: "USD")}</g:link></td>
					
						<td>${fieldValue(bean: schoolInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: schoolInstance, field: "schoolID")}</td>
